...
{{Form::model($salesorderItem,array('route' => array('salesorder.item.update', $salesorderItem->id), 'method' => 'POST','class' => 'needs-validation', 'novalidate')) }}
<div class="row">
    ...
    <div class="form-group col-md-6">
        ...
        {{ Form::number('quantity',null, array('class' => 'form-control quantity','required'=>'required', 'placeholder' => __('Enter Quantity'))) }}
    </div>
    <div class="form-group col-md-6">
        {{ Form::label('price', __('Price'),['class'=>'form-label']) }}<x-required></x-required>
        {{ Form::number('price',null, array('class' => 'form-control price','required'=>'required','stage'=>'0.01','placeholder' => __('Enter Price'))) }}
    </div>
    <div class="form-group col-md-6">
        {{ Form::label('discount', __('Discount'),['class'=>'form-label']) }}
        {{ Form::number('discount',null, array('class' => 'form-control discount', 'placeholder' => __('Enter Discount'))) }}
    </div>
    <div class="form-group col-md-12">
        {{ Form::label('tax', __('Tax'),['class'=>'form-label']) }}<x-required></x-required>
        {{ Form::hidden('tax',null, array('class' => 'form-control taxId','required'=>'required')) }}
        <div class="row">
            <div class="col-md-12">
                <div class="tax"></div>
            </div>
