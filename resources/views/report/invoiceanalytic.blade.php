                ...
                <div class="cardcard-body">
                    <div class="collapse show float-end" id="collapseExample" style="">
                        ...
                        <div class="row filter-css">
                            ...
                            <div class="col-auto">
                                {{ Form::select('account',$account,isset($_GET['account'])?$_GET['account']:'', array('class' => 'form-control ','style'=>'margin-right: 19px;')) }}
                            </div>
                            ...
                            <div class="action-btn  ms-5" style="margin-top :7px;margin-right :4px;">
                            <div class="col-auto">
                                <button type="submit" class="mx-3 bg-primary btn btn-sm align-items-center text-white" data-bs-toggle="tooltip" data-title="{{__('Apply')}}" title="{{__('Apply')}}"><i class="ti ti-search"></i></button>
                            </div>
                            </div>
                            <div class="action-btn  ms-2" style="margin-top :7px;margin-right :4px;">
                            <div class="col-auto">
                                <a href="{{route('report.invoiceanalytic')}}" data-bs-toggle="tooltip" data-title="{{__('Reset')}}" title="{{__('Reset')}}" class="mx-3 bg-danger btn btn-sm align-items-center text-white"><i class="ti ti-refresh"></i></a>
                            </div>
                            </div>
                            <div class="action-btn  ms-2" style="margin-top :7px; margin-right :4px;">
                            <div class="col-auto">
                                <a href="#" onclick="saveAsPDF();" class="mx-3 bg-primary btn btn-sm align-items-center text-white" data-bs-toggle="tooltip" data-title="{{__('Download')}}" title="{{__('Download')}}" id="download-buttons">
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-body table-border-style">
                    <div>
                        <button class="btn btn-light-primary btn-sm csv me-1">Export CSV</button>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($result->status == 0)
                                            <span
                                                class="badge status_badge bg-secondary p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @elseif($result->status == 1)
                                            <span
                                                class="badge status_badge bg-danger p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @elseif($result->status == 2)
                                            <span
                                                class="badge status_badge bg-warning p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @elseif($result->status == 3)
                                            <span
                                                class="badge status_badge bg-success p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @elseif($result->status == 4)
                                            <span
                                                class="badge status_badge bg-info p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @endif

                                        {{-- @if($result->status == 0)
                                            <span class="badge status_badge bg-info p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @elseif($result->status == 1)
                                            <span class="badge status_badge bg-info p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @elseif($result->status == 2)
                                            <span class="badge status_badge bg-info p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @elseif($result->status == 3)
                                            <span class="badge status_badge bg-success p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @elseif($result->status == 4)
                                            <span class="badge status_badge bg-warning p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @elseif($result->status == 5)
                                            <span class="badge status_badge bg-danger p-2 px-3">{{ __(\App\Models\Invoice::$status[$result->status]) }}</span>
                                        @endif --}}
                                    </td>
                                    <td class="">
